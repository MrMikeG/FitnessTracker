...
export default function Home() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 pb-28 pt-5 sm:px-7 sm:pt-8">
      ...
      <div className="mx-auto flex max-w-6xl items-center gap-4"><div className="min-w-max"><p className="text-[10px] font-bold uppercase tracking-[.15em] text-zinc-500">12-week progress</p><p className="text-sm font-bold text-white">Week {selectedWeek} <span className="font-medium text-zinc-500">of {plan.totalWeeks}</span></p></div><div className="h-2 flex-1 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-lime transition-all duration-500" style={{ width: `${(selectedWeek / plan.totalWeeks) * 100}%` }}/></div><span className="text-sm font-bold text-lime">{Math.round((selectedWeek / plan.totalWeeks) * 100)}%</span></div>
      </footer>
    </main>
  )
}
