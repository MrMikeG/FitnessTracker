...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (typeof progress.restPreset === 'number') setRestPreset(progress.restPreset)
        if (typeof progress.restEndsAt === 'number' && progress.restEndsAt > Date.now()) {
          setRestEndsAt(progress.restEndsAt)
          setRestRemaining(Math.ceil((progress.restEndsAt - Date.now()) / 1000))
          setRestRunning(true)
        } else if (typeof progress.restEndsAt === 'number') {
          setRestRemaining(0)
        } else if (typeof progress.restRemaining === 'number') setRestRemaining(progress.restRemaining)
      }
  ...
  useEffect(() => {
    if (!hydrated) return
    const progress: SavedProgress = { completedByDay, setProgressByDay, weightByExercise, completedWorkouts, skippedDays, selectedDayIndex, selectedWeek, dark, restPreset, restRemaining, restEndsAt }
    window.localStorage.setItem(storageKey, JSON.stringify(progress))
  }, [completedByDay, completedWorkouts, dark, hydrated, restEndsAt, restPreset, restRemaining, selectedDayIndex, selectedWeek, setProgressByDay, skippedDays, weightByExercise])
  ...
  const chooseRestPreset = (seconds: number) => { setRestPreset(seconds); setRestRemaining(seconds); setRestRunning(false); setRestEndsAt(null) }
  const toggleRestTimer = () => {
    ...
    const seconds = restRemaining === 0 ? restPreset : restRemaining
  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-4 pb-40 pt-5 sm:px-7 sm:pt-8">
      ...
      <section className="glass mb-5 rounded-3xl p-5 sm:p-6">
        ...
        <div className="mt-5 grid grid-cols-[1fr_auto] gap-2"><button onClick={toggleRestTimer} className="flex items-center justify-center gap-2 rounded-2xl bg-ink py-3 text-sm font-bold text-white transition hover:scale-[1.01] dark:bg-white dark:text-ink">{restRunning ? <><Pause size={17} fill="currentColor"/> Pause</> : <><Play size={17} fill="currentColor"/> {restRemaining === 0 ? 'Restart timer' : 'Start rest'}</>}</button><button onClick={() => { setRestRemaining(restPreset); setRestRunning(false); setRestEndsAt(null) }} className="grid w-12 place-items-center rounded-2xl border border-zinc-200 text-zinc-500 transition hover:text-ink dark:border-white/10 dark:hover:text-white" aria-label="Reset rest timer"><RotateCcw size={18}/></button></div>
      </section>
      ...
      <footer className="fixed inset-x-3 bottom-[max(0.75rem,env(safe-area-inset-bottom))] z-20 rounded-2xl border border-white/10 bg-[#0c0e0c]/95 px-4 py-3 shadow-soft backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl flex-col gap-2 sm:flex-row sm:items-center sm:gap-4"><div className="flex flex-1 items-center gap-4"><p className="min-w-max text-xs font-bold uppercase tracking-[.15em] text-zinc-400">12-week progress</p><div className="h-2 flex-1 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-lime transition-all duration-500" style={{ width: `${overallProgress}%` }}/></div><span className="text-sm font-bold text-lime">{overallProgress}%</span></div><button onClick={toggleRestTimer} className={`flex items-center justify-center gap-2 rounded-xl px-3 py-2 text-sm font-bold transition ${restRunning ? 'bg-lime text-ink' : 'bg-white/10 text-white hover:bg-white/15'}`} aria-label={restRunning ? 'Pause rest timer' : restRemaining === 0 ? 'Restart rest timer' : 'Start rest timer'}><Timer size={15}/><span>Rest</span><span className="tabular-nums">{restTime}</span>{restRunning ? <Pause size={14} fill="currentColor"/> : <Play size={14} fill="currentColor"/>}</button></div>
      </footer>
    </main>
  )
}
